...
class VoidexClient(VDXUtils):
    def __init__(self, api_key: str, proxy:str = None, bot_token: str = None, chat_id: int = None):
        ...
        self.base_url = "https://api.voidapi.xyz/v2/{}"
        self.proxy = proxy or ""
    ...
    def stripe_auth(cls, card):
     try:
        # card length must be equal to 12 or greater 
        cards = cls.create_lista_(card)
        if not cards:
           return {"error":"No valid card found"}
        
        data = {
                'card': card, "key": cls.api_key
        }

        url = cls.base_url.format('stripe_auth')
     ...
     try:
        data = {
            ...
            'data': {
                ...
                "is_shippable": self.config.SHOPIFY_SHIPPABLE,
            }
        }
